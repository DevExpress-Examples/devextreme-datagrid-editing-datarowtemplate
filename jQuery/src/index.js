$(() => {
  ...
  const createEditRowTemplate = (container, item) => {
    ...
    const markup = createEditMarkup(data);
    container.append(markup);

    editors.forEach((editor) => {
      createEditor(editor.editorID, data[editor.dataField], editor.dataField, data.ID);
    });
  ...
  const createDataRowTemplate = (container, item) => {
    const markup = createDataMarkup(item.data);
    container.append(markup);
  ...
  const createEditor = (editorID, value, dataField, dataID) => {
    ...
    return isDateField ? $(editorID).dxDateBox(options) : $(editorID).dxTextBox(options);
  };

  const onValueChanged = (dataField, key) => ({ value }) => {
    if (!changes.length) {
      changes.push({ data: { [dataField]: value }, key, type: 'update' });
    } else {
  ...
  const createButtons = (container, item, button1, button2) => {
    ...
    $(container).find(`#editColumn-${item.data.ID}`).append($button1, $button2);
  };

  const createEditMarkup = (data) => "<tr class='main-row'>"
  ...
  const createDataMarkup = (data) => "<tr class='main-row'>"
    + `<td>${data.Prefix}</td>`
